...
- Parallel scanning via `concurrent.futures.ThreadPoolExecutor`
- TOCTOU-safe installation via `pip install --no-index --find-links`
- sdist detection by file extension — blocks silent `--prefer-binary` fallback
- Seed allowlist of 10 packages (boto3, keyring, paramiko, etc.) — reduces HIGH→MEDIUM
- CRITICAL findings never reduced by allowlist
...
- Guaranteed temp dir cleanup via `atexit` + SIGINT handler
